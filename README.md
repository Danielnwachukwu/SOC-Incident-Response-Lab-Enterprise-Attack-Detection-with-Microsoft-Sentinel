...
The investigation reconstructed the complete attack chain from infrastructure deployment through persistence and threat hunting.
...
![Azure VM](Images/01-Lab-Deployment/01_Azure_VM_Successful_Deployment.png)

Initial login to the monitored Windows Server.

![Windows Server Login](Images/01-Lab-Deployment/03_First_Login_Windows_Server_Azure.png)
...
Initial Nmap service enumeration performed against the Azure Windows Server.

![Nmap Enumeration](Images/02-Reconnaissance/06_Nmap_Initial_Port_and_Service_Enumeration.png)

Advanced TCP SYN and service fingerprinting used to identify exposed services.

![Advanced Nmap Scan](Images/02-Reconnaissance/10_Advanced_Nmap_Service_and_SYN_Scan.png)
---
...
Multiple Remote Desktop authentication failures were generated to simulate brute-force activity.

![RDP Failure](Images/03-Initial-Access/11_RDP_Login_Failure_User_Prompt.png)

Microsoft Sentinel detected Event ID 4625 authentication failures.

![4625 Timeline](Images/03-Initial-Access/13_Sentinel_Event4625_Failed_Logon_Timeline.png)

Successful interactive authentication was confirmed using Windows Event ID 4624.

![Successful Logon](Images/03-Initial-Access/15_Sentinel_Event4624_Successful_Logon_Validation.png)
---
...
PowerShell executed local discovery commands after successful authentication.

![PowerShell Discovery](Images/04-PowerShell-Execution/01_PowerShell_Local_Discovery_Commands.png)

Microsoft Sentinel reconstructed the complete PowerShell process lineage using Sysmon Event ID 1.

![PowerShell Lineage](Images/04-PowerShell-Execution/03_Sentinel_Process_Lineage_PowerShell_Whoami.png)

Network connections generated by PowerShell were identified through Sysmon Event ID 3.

![PowerShell Network](Images/04-PowerShell-Execution/05_Sentinel_Sysmon_Network_Connection_EventID3.png)
---
...
The attacker enumerated local accounts using the net user command.

![net user](Images/09-Threat-Hunting/27_Local_User_Enumeration_net_user.png)

Microsoft Sentinel captured the full command line used during account enumeration.

![Command Line Evidence](Images/09-Threat-Hunting/28_Sentinel_net_user_CommandLine_Evidence.png)
---
...
Registry Run Key persistence was established using PowerShell.

![Registry Persistence](Images/04-PowerShell-Execution/08_PowerShell_Create_Registry_RunKey_Persistence.png)

Sysmon Event ID 13 confirmed Registry value creation.

![Registry Detection](Images/04-PowerShell-Execution/10_Sentinel_Registry_RunKey_Event_Details.png)
---
...
Base64-encoded PowerShell commands were executed to evade simple command-line inspection.

![Encoded PowerShell](Images/04-PowerShell-Execution/13_PowerShell_EncodedCommand_Execution.png)

Microsoft Sentinel reconstructed the decoded process execution through Sysmon telemetry.

![Encoded Detection](Images/04-PowerShell-Execution/15_Sentinel_Encoded_PowerShell_Process_Details.png)
---
...
BITSAdmin was abused to download a file from the Internet.

![BITSAdmin Download](Images/04-PowerShell-Execution/16_BITSAdmin_File_Download_Execution.png)

Microsoft Sentinel detected the resulting process creation.

![BITSAdmin Detection](Images/04-PowerShell-Execution/17_Sentinel_BITSAdmin_Process_Creation.png)
---
...
A malicious scheduled task was created to execute PowerShell at logon.

![Task Creation](Images/09-Threat-Hunting/21_Scheduled_Task_Creation_PowerShell.png)

Microsoft Sentinel detected schtasks.exe creating the persistence mechanism.

![Task Detection](Images/09-Threat-Hunting/22_Sentinel_Scheduled_Task_Process_Creation.png)

PowerShell execution triggered from the scheduled task.

![Task Execution](Images/09-Threat-Hunting/24_Sentinel_PowerShell_Process_From_Scheduled_Task.png)
---
...
Threat hunting correlated authentication events, PowerShell execution, Registry modifications, BITSAdmin activity, Scheduled Tasks, and discovery commands to reconstruct the complete attack timeline.

![Threat Hunting](Images/09-Threat-Hunting/26_Sentinel_Local_Account_Discovery.png)
---
...
The investigation reconstructed the intrusion chronologically by correlating Windows Security Events with Sysmon telemetry inside Microsoft Sentinel.

Evidence was collected through:

- Windows Security Event IDs 4624 and 4625
- Sysmon Event IDs 1, 3, 11 and 13
- Parent-child process lineage
- Command-line telemetry
- Registry modifications
- File creation events
- Network connection telemetry
- Threat hunting queries written in Kusto Query Language (KQL)
